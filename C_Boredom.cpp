...
int Bore(int i,int n,vector<int>& v,int dp[],mi& fre){

    if(i>=n) return 0;
    if(dp[i]!=-1) return dp[i];
    
        if(i<n-1&&v[i]==v[i+1]-1)
            dp[i]= max(Bore(i+1,n,v,dp,fre),fre[v[i]]+Bore(i+2,n,v,dp,fre));
        else
            dp[i]= fre[v[i]]+Bore(i+1,n,v,dp,fre);
    return dp[i];
}
