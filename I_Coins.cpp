...
int32_t main(){
    ...
    int n; cin>>n;
    vector<long double> prob(n); cin>>prob;
    long double dp[n][n+1];
    f(i,0,n)
        f(j,0,n+1) dp[i][j]=0.000000;
        
    // f(i,0,n){
    //     f(j,0,n+1) cout<<dp[i][j]<<" ";
    //     cout<<endl;
    // }
    
    dp[0][0]=1-prob[0]; dp[0][1]=prob[0];
    f(i,1,n){
        long double head=prob[i],tail=1-prob[i];
        dp[i][0]=dp[i-1][0]*tail;
        fe(j,1,n){
            dp[i][j]=(dp[i-1][j-1]*head)+(dp[i-1][j]*tail);
        }
    }
    long double ans=0;
