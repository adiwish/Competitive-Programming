...
bool check(int k,vector<pair<int,int>>& v){
    ...
    f(i,0,n){
        curr=intersection(curr,v[i]);
        if(curr.first>curr.second) return false;
        curr.first-=k; curr.second+=k;
    }
    return true;
}
...
void code(){
    ...
    while(l<=h){
        int mid=(l+h)/2;
        bool midpos=check(mid,v);
        bool midprevpos=check(mid-1,v);
        if(midpos&&midprevpos) h=mid-1;
        else if(!midpos) l=mid+1;
        else{
            cout<<mid; ret;
        }
    }
