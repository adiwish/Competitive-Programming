...
void code(){
    ...
    int pre[n]={0};
    f(i,1,n){
        if(a[i]<a[i-1]) pre[i]=a[i-1]-a[i];
    }
    int suff[n]={0};
    fr(i,n-2,0){
        if(a[i]<a[i+1]) suff[i]=a[i+1]-a[i];
    }
    ...
    while(m--){
        ...
        if(x<y){
            if(x==1) cout<<pre[y-1];
            else cout<<pre[y-1]-pre[x-1];
        }
        else{
            if(x==n) cout<<suff[y-1];
            else cout<<suff[y-1]-suff[x-1];
        }
        cout<<endl;
    }
