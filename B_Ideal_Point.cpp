...
void code(){
    int n,k; cin>>n>>k;
    bool ans=false;
    bool leftk=false,rightk=false,centre=false,right=false;
    f(i,0,n){
        int x,y; cin>>x>>y;
        if(x==y&&x==k) ans=true;
        if(y<k||x>k) continue;
        // if(x<k&&y>k) centre=true;
        if(x<k&&y==k) leftk=true;
        if(x==k&&y>k) rightk=true;
    }
    // if(centre&&(rightk||leftk)) cout<<"YES";
    if(ans) cout<<"YES";
    else if(leftk&&rightk) cout<<"YES";
    else cout<<"NO";
