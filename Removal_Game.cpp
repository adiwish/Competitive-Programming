...
int cal(int i,int j,vi& v,vvi& dp){
    if(i>j) return 0;
    if(dp[i][j]!=-1) ret dp[i][j];
    int ans1=v[i]+min(cal(i+2,j,v,dp),cal(i+1,j-1,v,dp));
    int ans2=v[j]+min(cal(i+1,j-1,v,dp),cal(i,j-2,v,dp));
    ret dp[i][j]=max(ans1,ans2);
