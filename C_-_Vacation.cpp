...
int solve(int i,int last,vector<vector<int>>& v,vector<vector<int>>& dp){
    if(i<0) return 0;
    if(dp[i][last]!=-1&&last!=-1) return dp[i][last];
    int temp=-1;
    for(int j=0;j<3;j++){
        if(j==last) continue;
        temp=max(temp,v[i][j]+solve(i-1,j,v,dp));
    }
    if(last!=-1)
        dp[i][last]=temp;
    return temp;
