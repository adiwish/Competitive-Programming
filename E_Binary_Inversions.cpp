...
void code(){
    ...
    int one[n]={0},zero[n]={0};
    one[0]=a[0];
    f(i,1,n){
        one[i]=a[i]+one[i-1];
    }
    if(a[n-1]==0) zero[n-1]=1;
    for(int i=n-2;i>=0;i--){
        if(a[i]==0) zero[i]=1+zero[i+1];
    ...
    for(int i=1;i<n-1;i++){
        if(a[i]==1){
            ans=max(ans,one[i-1]-zero[i+1]);
        }
        else{
            ans=max(ans,zero[i+1]-one[i-1]);
        }
    }
    if(a[0]==0) ans=max(ans,zero[1]);
    if(a[n-1]==1) ans=max(ans,one[n-2]);
    for(int i=0;i<n-1;i++){
        if(a[i]==1){
            ans+=zero[i+1];
        }
