...
double nthRoot(double number, int n) {
    if (number < 0 && n % 2 == 0) {
        std::cout << "Error: Even root of a negative number is undefined.\n";
        return 0.0;
    }
    
    return std::pow(number, 1.0 / n);
}
int cal(int mid,int t){
    int ans=1,k=mid;
    for(int i=2;i<=t;i++){
        ans+=k;
        k*=mid;
...
bool binary(int t,int n){
    int l=2,h=ceil(nthRoot(n,t-1));
    // if(t==3)
        // cout<<"t= "<<t<<"  l= "<<l<<"  h= "<<h<<endl;
    while(l<=h){
        int mid=(l+h)/2;
        int sum=cal(mid,t);
        // cout<<mid<<" - "<<sum<<endl;
